...
function PromoterForm({
  ...
  useEffect(() => {
    if (stateCode) {
      const load = async () => {
        ...
        const items = await getGstDistricts(stateCode);
        setDistrictItems(items);

        // HIGH-LEVEL NORMALIZATION: Ensures saved Name matches dropdown Code
        const normalized = normalizeDistrict(data[s("district_res")], items);
        if (normalized) update(s("district_res"), normalized);
  ...
  // PIN Code Auto-fill — fixed dependency array (no functions/arrays, just the pin value)
  useEffect(() => {
    const pin = data[s("pin_code")];
    if (pin?.length === 6 && (data[s("country")] || "IN") === "IN") {
      const loadAddress = async () => {
        const address = await fetchAddressByPin(pin);
        if (address) {
          const allStates = getStatesForCountry("IN");
          const matchedState = allStates.find(
            (st) => st.label.toLowerCase() === address.stateName.toLowerCase()
          );
          if (matchedState) {
            update(s("state_res"), matchedState.value);
            // Hybrid mapping: Try to find the code for the name from the fetched list
            const items = await getGstDistricts(matchedState.value);
            const found = items.find(
              (i) => i.label.toLowerCase() === address.district.toLowerCase()
            );
            update(s("district_res"), found ? found.value : address.district);
  ...
  // Citizenship toggle → auto-set/clear Country field
  useEffect(() => {
      ...
      {suffix === "" && isAlsoSignatory && (
        ...
        >
          ...
          <span style={{ fontSize: 12.5, fontWeight: 600, color: "#065F46" }}>
            <strong>Auto-Sync Enabled:</strong> Details synced to Authorized
            Signatory below.
          </span>
        </div>
      )}
      ...
      <SectionCard title="Personal Information" icon="👤">
        <Grid3>
          ...
          <FormInput
            ...
            {...f("father_first")}
            placeholder="Father's first"
          />
          <FormInput
            label="Father's Middle Name"
            {...f("father_middle")}
            placeholder="Father's middle"
          />
          <FormInput
            label="Father's Last Name"
            {...f("father_last")}
            placeholder="Father's last"
          />
        </Grid3>
        <Grid3>
        ...
        <Grid2>
          <FormInput
            label="Telephone (STD Code)"
            {...f("telephone")}
            placeholder="022-23456789"
          />
          <div>
            <FormRadioGroup
              label="Gender"
      ...
      <SectionCard title="Residential Address" icon="🏠">
        ...
        <Grid3>
          <FormSelect
            ...
            {...sel("district_res")}
            items={districtItems}
            disabled={!data[s("state_res")] || loadingDistricts}
  ...
  useEffect(() => {
    if (stateCode) {
      const load = async () => {
        ...
        const normalized = normalizeDistrict(data.as_district, items);
        if (normalized) update("as_district", normalized);
      ...
      const loadAddress = async () => {
        ...
        if (address) {
          ...
          if (matchedState) {
            ...
            update("as_district", found ? found.value : address.district);
  ...
  return (
    <>
      {data["Also Authorized Signatory"] && (
        <InfoAlert type="info">
          <strong>Note:</strong> Auto-filled from Promoter 1 ("Also Authorized
          Signatory" is ON). Changes here are saved for signatory only.
        </InfoAlert>
      )}
      ...
      <SectionCard title="Authorized Signatory Details" icon="✍️">
        ...
        <Grid2>
          ...
          <div>
            <FormRadioGroup
              label="Gender"
              value={data.radiogroup_1}
              onChange={(v) => update("radiogroup_1", v)}
              items={[
                { value: "Male", label: "Male" },
                { value: "Female", label: "Female" },
                { value: "Others", label: "Others" },
      ...
      <SectionCard title="Signatory Identity" icon="🪪">
        ...
        <Grid3>
          <div>
            <FormToggle
              label="Citizen / Resident of India?"
              value={!!data.toggle_3}
              onChange={(v) => update("toggle_3", v)}
            />
          </div>
          <FormInput
            label="Passport No. (Foreigners)"
      ...
      <SectionCard title="Signatory Residential Address" icon="🏠">
        <InfoAlert>
          Ensure addresses match your proof documents exactly.
        </InfoAlert>
        <Grid3>
          <FormSelect
            label="Country"
            value={data.as_country}
            onChange={(e) => update("as_country", e.target.value)}
            items={COUNTRIES}
          />
          <FormInput
            label="PIN Code"
          ...
          <FormSelect
            ...
            items={stateItems}
            onChange={(e) => {
              update("as_state", e.target.value);
              update("as_district", "");
            }}
          />
        </Grid3>
        <Grid3>
          <FormSelect
            ...
            {...sel("as_district")}
            items={districtItems}
            disabled={!data.as_state || loadingDistricts}
            hint={loadingDistricts ? "⏱️ Loading Codes..." : ""}
          />
          <FormInput
            label="City / Town / Village"
            value={data.as_city ?? ""}
            onChange={(e) => update("as_city", e.target.value || null)}
            placeholder="City"
          />
          <FormInput
            label="Locality / Sub Locality"
            value={data.as_locality ?? ""}
            onChange={(e) => update("as_locality", e.target.value || null)}
            placeholder="Locality"
          />
        </Grid3>
        <Grid3>
          <FormInput
            label="Road / Street"
            value={data.as_road ?? ""}
            onChange={(e) => update("as_road", e.target.value || null)}
            placeholder="Road or street"
          />
          <FormInput
            label="Premises / Building Name"
            value={data.as_premises ?? ""}
            onChange={(e) => update("as_premises", e.target.value || null)}
            placeholder="Building name"
          />
          <FormInput
            label="Building No. / Flat No."
            value={data.as_bno ?? ""}
            onChange={(e) => update("as_bno", e.target.value || null)}
        ...
        <Grid2>
          <FormInput
            label="Floor No."
            value={data.as_floor ?? ""}
            onChange={(e) => update("as_floor", e.target.value || null)}
            placeholder="Floor number"
          />
          <FormInput
            label="Nearby Landmark"
            value={data.as_landmark ?? ""}
            onChange={(e) => update("as_landmark", e.target.value || null)}
            placeholder="Nearby landmark"
          />
        </Grid2>
      </SectionCard>
      ...
      <SectionCard title="Signatory Documents" icon="📎">
        <Grid3>
          <FormSelect
            label="Proof of Authorized Signatory"
            value={data.as_proof_type}
            onChange={(e) => update("as_proof_type", e.target.value)}
            items={AUTH_SIGNATORY_PROOF}
          />
          <div style={{ gridColumn: "span 2" }}>
            <Grid2>
              <FileInput
                label="Upload Proof (PDF/JPEG, max 1MB)"
                value={data.as_proof_file}
                onChange={(v) => update("as_proof_file", v)}
                maxKb={1024}
              />
              <FileInput
                label="Photograph (JPEG only, max 100KB)"
                value={data.as_photo}
                onChange={(v) => update("as_photo", v)}
                maxKb={100}
                forceJpeg={true}
              />
            </Grid2>
...
function AuthRepSection({ data, update, errors, touched, touch }) {
  ...
  const [repWasToggled, setRepWasToggled] = useState(false);

  useEffect(() => {
    // Only clear if the user INTENTIONALLY turned this off (not on initial load/draft population)
    if (!data.toggle_4 && repWasToggled) {
      const keysToClear = [
        "radiogroup_2",
        "enrolment_id",
        "rep_name_first",
  ...
  }, [data.toggle_4, update, repWasToggled]);
  ...
  return (
    <SectionCard title="Authorized Representative" icon="👔">
      <FormToggle
        label="Do you have any Authorized Representative?"
        value={!!data.toggle_4}
        onChange={(v) => {
          setRepWasToggled(true);
          update("toggle_4", v);
        }}
      />
      ...
      {data.toggle_4 && (
        <div className="field-animate">
          <FormRadioGroup
            label="Type of Authorised Representative"
            value={data.radiogroup_2}
            onChange={(v) => update("radiogroup_2", v)}
            items={[
              { value: "GST Practitioner", label: "GST Practitioner" },
          ...
          <Grid2>
            <FormInput
              label="Enrolment ID"
              value={data.enrolment_id}
              onChange={(e) => update("enrolment_id", e.target.value)}
              placeholder="e.g. TRP123456789012"
              disabled={isOther}
            />
            <div />
          </Grid2>
          ...
          <Grid3>
            ...
            <FormInput
              label="Middle Name"
              value={data.rep_name_middle || ""}
              onChange={(e) => update("rep_name_middle", e.target.value)}
              placeholder="Middle name"
              disabled={isGSTP}
            />
            <FormInput
          ...
          <Grid3>
            <FormSelect
              ...
              onChange={(e) => update("rep_designation", e.target.value)}
              items={REP_DESIGNATIONS}
              disabled={isGSTP}
            />
            <FormInput
              label="Mobile +91"
          ...
          <Grid3>
            ...
            <FormInput
              label="Aadhaar Number"
              value={data.rep_aadhaar ?? ""}
              onChange={(e) => update("rep_aadhaar", e.target.value || null)}
              placeholder="12-digit Aadhaar"
              disabled={isGSTP || isOther}
            />
            <FormInput
              label="Telephone (STD Code)"
              value={data.rep_telephone || ""}
              onChange={(e) => update("rep_telephone", e.target.value)}
              placeholder="022-23456789"
              disabled={isGSTP}
            />
          </Grid3>
          ...
          <Grid2>
            <FormInput
              ...
              value={data.rep_fax ?? ""}
              onChange={(e) => update("rep_fax", e.target.value || null)}
              placeholder="022-23456789"
              disabled={isGSTP}
            />
            <div />
  ...
  return (
    <div className="section-container animate-fade-in">
      ...
      <InfoAlert>
        ...
        auto-sync to the Authorized Signatory section below.
      </InfoAlert>
      ...
      >
        ...
        >
          <span style={{ fontSize: 22, lineHeight: 1 }}>+</span> Add Another
          Promoter / Partner
        </button>
      </div>
      ...
      <div
        style={{ borderTop: "3px solid #E2E8F0", paddingTop: 20, marginTop: 4 }}
      >
        <div
          style={{
            display: "flex",
