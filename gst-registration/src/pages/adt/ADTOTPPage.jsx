...
export default function ADTOTPPage() {
  const navigate = useNavigate();
  const contactInfo = (() => { try { return JSON.parse(localStorage.getItem("adt_contact"))||{}; } catch { return {}; } })();
  ...
  const TimerBadge = ({ timer, onResend, verified }) => (
    <div style={{ textAlign:"center", marginTop:10 }}>
      {verified ? <span style={{ fontSize:13, color:"#059669", fontWeight:700 }}>✓ Verified successfully</span>
        : timer>0 ? <span style={{ fontSize:12.5, fontWeight:600, color:timer<=10?"#DC2626":"#94A3B8" }}>Resend OTP in {String(Math.floor(timer/60)).padStart(2,"0")}:{String(timer%60).padStart(2,"0")}</span>
        : <button onClick={onResend} style={{ background:"none", border:"none", color:"#0F172A", fontSize:13, fontWeight:700, cursor:"pointer", textDecoration:"underline" }}>Resend OTP</button>}
    </div>
  );
    ...
    <div style={{ minHeight:"100vh", background:"#F4F6FA", display:"flex", alignItems:"center", justifyContent:"center", padding:16 }}>
      <div className="section-card-content" style={{ background:"#fff", borderRadius:20, border:"1px solid #E2E8F0", maxWidth:520, width:"100%", boxShadow:"0 8px 40px rgba(15,23,42,0.1)", animation:"fadeInUp 0.35s ease both" }}>
        <div style={{ textAlign:"center", marginBottom:30 }}>
          <div style={{ width:52, height:52, borderRadius:14, background:"linear-gradient(135deg,#0F172A,#334155)", display:"flex", alignItems:"center", justifyContent:"center", margin:"0 auto 14px", boxShadow:"0 6px 18px rgba(15,23,42,0.2)" }}>
          ...
          <h2 style={{ fontSize:22, fontWeight:800, color:"#1E293B", marginBottom:6 }}>ADT-1 OTP</h2>
          <p style={{ fontSize:13, color:"#64748B", lineHeight:1.65 }}>OTPs sent to <strong style={{ color:"#0F172A" }}>+91 {contactInfo.mobile}</strong> and <strong style={{ color:"#0F172A" }}>{contactInfo.email}</strong></p>
        </div>

        {/* Mobile OTP */}
        {[{label:"Mobile OTP",sub:`+91 ${contactInfo.mobile}`,otp:mobileOtp,setOtp:setMobileOtp,refs:mobileRefs,verified:mobileVerified,error:mobileError,timer:mobileTimer,onResend:()=>setMobileTimer(60),onVerify:verifyMobile,shakeKey:"mobile",color:"#0F172A",bg:"#F1F5F9",icon:<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 013.07 10.8a19.79 19.79 0 01-3.07-8.68A2 2 0 012 0h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L6.09 7.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 14.92v2z"/></svg>,btnBg:"#0F172A"},
          {label:"Email OTP",sub:contactInfo.email,otp:emailOtp,setOtp:setEmailOtp,refs:emailRefs,verified:emailVerified,error:emailError,timer:emailTimer,onResend:()=>setEmailTimer(60),onVerify:verifyEmail,shakeKey:"email",color:"#334155",bg:"#F8FAFC",icon:<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>,btnBg:"#334155"},
        ].map((card) => (
          <div key={card.label} style={{ background:"#F8FAFC", border:`1.5px solid ${shake[card.shakeKey]?"#FCA5A5":card.verified?"#BBF7D0":"#E2E8F0"}`, borderRadius:12, padding:"20px 20px 16px", marginBottom:14 }}>
            ...
            {!card.verified && (
              <button onClick={card.onVerify} className="nav-btn"
                style={{ width:"100%", marginTop:12, padding:10, fontSize:13.5, fontWeight:700, background:card.btnBg, color:"#fff", border:"none", borderRadius:9, cursor:"pointer" }}>
                Verify {card.label}
              </button>
            )}
          </div>
        ...
        <button onClick={()=>{ if(!canProceed)return; localStorage.setItem("adt_otp_verified","1"); navigate("/adt/form"); }} className="nav-btn"
          style={{ width:"100%", padding:13, fontSize:15, fontWeight:800, background:canProceed?"#059669":"#F1F5F9", color:canProceed?"#fff":"#94A3B8", border:`1.5px solid ${canProceed?"transparent":"#E2E8F0"}`, borderRadius:12, cursor:canProceed?"pointer":"not-allowed", display:"flex", alignItems:"center", justifyContent:"center", gap:9 }}>
          {canProceed ? <>Proceed to ADT Form <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="9 18 15 12 9 6"/></svg></> : "Verify at least one OTP to continue"}
        </button>
        <button onClick={()=>navigate("/adt")} style={{ width:"100%", marginTop:10, padding:10, background:"none", border:"none", color:"#94A3B8", fontSize:13, cursor:"pointer", fontWeight:500 }}>← Back to contact details</button>
      </div>
    </div>
  );
}
